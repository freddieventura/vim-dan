...
DOWNLOAD_LINK="https://nextjs.org"
# -------------------------------------
...
indexing_rules(){
    ...
    `## Recursive Accept/Reject Options` \
      --no-parent \
      --reject-regex '.*[%&=?].*|.*?hel=.*|.*?hl=.*' \
      --reject '*.ico,*.woff2,*.java,*.sql,*.jar,*.jpg,*.zip,*.GIF,*mp4,*.gif,*.svg,*.js,*json,*.css,*.PNG,*.png,*.xml,*.txt' \
      --page-requisites \
      ${DOWNLOAD_LINK}
}
...
mv ${DOCU_PATH}/downloaded/nextjs.org/* ${DOCU_PATH}/downloaded/
rm -r ${DOCU_PATH}/downloaded/nextjs.org
...
rm -r ${DOCU_PATH}/downloaded/conf
...
## There are many duplicate files in the documentation, delete them
jdupes -d -N "${DOCU_PATH}/downloaded/"
...
for file in "${files_array[@]}"; do
    ...
    f1() { pup -i 0 --pre 'h1' | pandoc -f html -t plain | sed ':a;N;$!ba;s/\n/ /g';}
...
for path in "${files_array[@]}"; do
    ...
    # (Multi-rule) Parsing functions , add as many as you want
    f1() { pup -i 0 --pre 'article' | pandoc -f html -t plain --wrap=none;}
