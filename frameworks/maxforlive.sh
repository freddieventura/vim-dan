...
DOWNLOAD_LINK="https://docs.cycling74.com/max8"
# -------------------------------------
...
`## HTTP Options` \
  --user-agent="Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chro  me/91.0.4472.124 Safari/537.36 Edg/91.0.864.59" \
  --adjust-extension \
`## HTTPS Options` \
...
mapfile -t files_array < <(find ${DOCU_PATH}/downloaded/ -type f -name "*" | awk '{print gsub(/\//,"/")"|"$0; }' | sort -t'|' -k1,1n -k2 | sed 's/^[^|]*|//')
    ...
    for file in "${files_array[@]}"; do
        ...
        # creating link_to
        parentname="$(basename "$(dirname "$file")")"
        link_to=$(cat ${file} | pup -i 0 --pre 'article h1' | pandoc -f html -t plain )
        echo "# ${parentname} ${link_to} #" >> ${MAIN_TOUPDATE}  ## Actual link_to
        echo ${link_to} | figlet >> ${MAIN_TOUPDATE}
        cat ${file} | pup -i 0 --pre 'nav.menu' | pandoc -f html -t plain >> ${MAIN_TOUPDATE}
        cat ${file} | pup -i 0 --pre 'div.c74-doc-content' | pandoc -f html -t plain 2>/dev/null >> ${MAIN_TOUPDATE}
        echo "-----------------------------------------------------------" >> ${MAIN_TOUPDATE}
    done
}
