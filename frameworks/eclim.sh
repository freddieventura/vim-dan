...
for DOWNLOAD_LINK in "${DOWNLOAD_LINKS[@]}"; do
    ...
    `## Recursive Accept/Reject Options` \
      --no-parent \
      --reject '*.gif,*.pdf,*.woff,*.woff2,*.ttf,*.png,*.webp,*.mp4,*.ico,*.svg,*.js,*json,*.css,*.xml,*.txt' \
      --page-requisites \
      ${DOWNLOAD_LINK}
done 
}
...
rm -r ${DOCU_PATH}/downloaded/_images

mv ${DOCU_PATH}/downloaded/eclim.org/* ${DOCU_PATH}/downloaded/
rmdir ${DOCU_PATH}/downloaded/eclim.org
...
for path in "${sorted_paths_array[@]}"; do
    ...
    # (Multi-rule) Parsing functions , add as many as you want
    f1() { pup -i 0 --pre 'div.content' | pandoc -f html -t plain | sed 's/[[:space:]]\+¶//g';}
