...
for i in {1..8}; do
    ...
    mapfile -t files_array < <(find "${DOCU_PATH}/downloaded/www.mongodb.com/docs/" -mindepth 2 -type f)
...
# Structuring first parent of directory hierachy

base_dir="${DOCU_PATH}/downloaded/www.mongodb.com/docs"
...
find "$base_dir" -type f | while read -r file; do
    ...
    if [[ "$filename" =~ ([^\)]+)-\)(.+) ]]; then
        ...
        new_filename="${BASH_REMATCH[2]}"

        # Determine the parent directory of the current file's directory
        ...
        # Move and rename the file to the new subdirectory
        mv "$file" "$target_dir/$new_filename"
    fi
done
# eof eof eof Structuring first parent of directory hierachy
...
for path in "${sorted_paths_array[@]}"; do
    ...
    # (Multi-rule) Parsing functions , add as many as you want
    f1() { pup -i 0 --pre 'section' | pandoc -f html -t plain ;}
