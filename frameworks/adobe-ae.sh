...
DOWNLOAD_LINK="https://ae-scripting.docsforadobe.dev/"
# -------------------------------------
...
# eof eof eof Parsing our own index for docu
    ...
    for file in "${dirs_array[@]}"; do
        ...
        for file in "${files_array[@]}"; do
            ...
            cat ${file} | pup -i 0 --pre 'li.current' | pandoc -f html -t plain >> ${MAIN_TOUPDATE}

            cat ${file} | pup -i 0 --pre '.rst-content' | pandoc -f html -t plain | sed 's/¶/#/g'  >> ${MAIN_TOUPDATE}
        done
    done
}
