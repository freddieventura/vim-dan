...
command! GotoJavaspecsLinkto call GotoJavaspecsLinktoFn()
command! IsLineJavaspecsLinkto call IsLineJavaspecsLinktoFn()
nnoremap <expr> <C-]> ( IsLineLinktoFn() ? ':GotoLinkto<CR>' : ( IsLineJavaspecsLinktoFn() ? ':GotoJavaspecsLinkto<CR>' : '<C-]>'))
...
def GotoJavaspecsLinktoFn(): void
    var myString = getline('.')

    # Chopping string from patternList elements
    # MODIFY LINE BELOW MODIFY LINE BELOW !!
    var trimPatternList = ['\s(X)']
