...
syn match danOutputs "^OUTPUTS\%(\s(X)\)\{,1}$" contains=danX
hi def link danOutputs TabLine

syn match danSyntax "^SYNTAX\%(\s(X)\)\{,1}$" contains=danX
hi def link danSyntax TabLine

syn match danSynopsis "^SYNOPSIS\%(\s(X)\)\{,1}$" contains=danX
hi def link danSynopsis TabLine

syn match danRelated "^RELATED\%(\s(X)\)\{,1}$" contains=danX
hi def link danRelated TabLine

syn match danInputs "^INPUTS\%(\s(X)\)\{,1}$" contains=danX
hi def link danInputs TabLine

syn match danCommonParam "^CommonParameters\%(\s(X)\)\{,1}$" contains=danX
hi def link danCommonParam Comment

syn match danNotes "^NOTES\%(\s(X)\)\{,1}$" contains=danX
hi def link danNotes Tabline

syn match danRelated "^RELATED LINKS\%(\s(X)\)\{,1}$" contains=danX
hi def link danRelated Tabline

syn match danSingleParam "^-\w\+\%(\s(X)\)\{,1}$" contains=danX
hi def link danSingleParam PmenuThumb
...
syn include @danPowershell syntax/ps1.vim
unlet b:current_syntax

syn region javaScript start=/[\.:]$\n\n\s\{4}/ms=s+5 keepend end=/.$\n\n^\S/me=e-4 contains=@danPowershell
